...
8. 現在地セクションで位置情報の取得結果を確認可能
9. 道路情報投稿セクションで説明文と画像を投稿し、最新の投稿を一覧表示
...
## GoogleAuth を有効化する方法 (概要)

このプロジェクトには Google 認証の実装は含まれていません。導入する場合は以下を参考にしてください。

1. Google Cloud Console でプロジェクトを作成し、OAuth 同意画面を設定する
2. OAuth クライアント ID (Web アプリ) を作成し、許可済みの JavaScript オリジンとリダイレクト URI を登録する
3. フロントエンドに Google Identity Services を読み込み、取得したクライアント ID を設定する
4. 取得した ID トークンをサーバー側で検証し、必要に応じてセッションやユーザー情報を管理する
